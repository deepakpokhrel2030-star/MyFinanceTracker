...
export const routes: Routes = [
  ...
  { path: 'analytics/:section', canActivate: [authGuard], loadComponent: () => import('./components/analytics/analytics').then(m => m.AnalyticsComponent) },

  { path: 'profile', canActivate: [authGuard], loadComponent: () => import('./components/profile/profile').then(m => m.ProfileComponent) },
  // Admin requires both authGuard (logged in) and adminGuard (has admin role)
  { path: 'admin', canActivate: [authGuard, adminGuard], loadComponent: () => import('./components/admin/admin').then(m => m.AdminComponent) },

  // Catch-all redirects unknown paths to dashboard instead of showing a blank page
