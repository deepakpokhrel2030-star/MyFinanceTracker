...
def create_budget():
    ...
    saved = ext.mongo.budgets.find_one({
        "user_id": uid,
        "month": doc["month"],
        "category": doc["category"]
    })

    return created({"message": "budget saved", "budget": serialize_budget(saved)})
...
def list_budgets():
    ...
    for budget in ext.mongo.budgets.find(q).sort([("month", -1), ("category", 1)]):
        items.append(serialize_budget(budget))
...
def update_budget(budget_id):
    ...
    if "month" in data:
        try:
            datetime.strptime(data["month"], "%Y-%m")
            allowed["month"] = str(data["month"]).strip()
        except Exception:
            return bad_request("month must be YYYY-MM format e.g. 2024-01")
    ...
    updated = ext.mongo.budgets.find_one({"_id": bid, "user_id": uid})

    return ok({"updated": True, "budget": serialize_budget(updated)})
