...
def _clean_doc(doc):
    ...
    for field in ["date", "created_at", "updated_at", "target_date", "deadline"]:
...
def get_user(user_id):
    ...
    transactions = [
        _clean_doc(t)
        for t in ext.mongo.transactions.find({"user_id": uid}).sort("date", -1)
    ]

    savings = [
        _clean_doc(s)
        for s in ext.mongo.savings_goals.find({"user_id": uid})
    ]

    investments = [
        _clean_doc(i)
        for i in ext.mongo.investments.find({"user_id": uid})
    ]

    budgets = [
        _clean_doc(b)
        for b in ext.mongo.budgets.find({"user_id": uid})
    ]

    for investment in investments:
        quantity = float(investment.get("quantity", 0) or 0)
        current_price = float(investment.get("current_price", 0) or 0)
        purchase_price = float(investment.get("purchase_price", 0) or 0)

        investment["current_value"] = round(quantity * current_price, 2)
        investment["invested_value"] = round(quantity * purchase_price, 2)

    user["accounts"] = accounts
    user["transactions"] = transactions
    user["savings"] = savings
    user["savings_goals"] = savings
    ...
    user["summary"] = {
        ...
        "total_income": round(sum(t.get("amount", 0) or 0 for t in transactions if (t.get("amount", 0) or 0) > 0), 2),
        "total_expense": round(sum(abs(t.get("amount", 0) or 0) for t in transactions if (t.get("amount", 0) or 0) < 0), 2),
        "total_savings": round(sum(s.get("current_amount", 0) or 0 for s in savings), 2),
        "total_savings_target": round(sum(s.get("target_amount", 0) or 0 for s in savings), 2),
        "total_investments": round(sum(i.get("current_value", 0) or 0 for i in investments), 2)
    }

    return ok({"user": user})


@bp.get("/admin/user-summary/<user_id>")
@jwt_required
@admin_required
...
def update_user(user_id):
    ...
    if "email" in data:
        allowed["email"] = str(data["email"]).lower().strip()

    if "phone" in data:
        allowed["phone"] = str(data["phone"]).strip()

    if "address" in data:
        allowed["address"] = data["address"]
...
def admin_all_transactions():
    ...
    items = [_clean_doc(t) for t in cursor]

    return ok({
        "items": items,
        "page": page,
        "size": size,
        "total": ext.mongo.transactions.count_documents({})
    })
...
@admin_required
def admin_analytics():
    bal = list(ext.mongo.accounts.aggregate([
        {"$group": {"_id": None, "total": {"$sum": "$balance"}}}
    ]))

    inc = list(ext.mongo.transactions.aggregate([
        {"$match": {"amount": {"$gt": 0}}},
        {"$group": {"_id": None, "total": {"$sum": "$amount"}}}
    ]))

    exp = list(ext.mongo.transactions.aggregate([
        {"$match": {"amount": {"$lt": 0}}},
